...
namespace SwfLibrary.Abc.Constants
{
    public class MultinameInfo : IExternalizeable
    {
        ...
        public const byte TypeName = 0x1d;
        ...
        public void ReadExternal(BinaryReader input)
        {
            ...
            switch (_kind)
            {
                ...
                case MultinameLA:
                    ...
                    break;
                case TypeName :
                    U30 name = Primitives.ReadU30(input);
                    U30 len = Primitives.ReadU30(input);
                    uint n=len.Value;
                    ...
                    _data[1] = len;
                    for (uint i = 0; i < n; ++i){
                        _data[i + 2] = Primitives.ReadU30(input);
                    }
                    break;
                default :
        ...
        public void WriteExternal(BinaryWriter output)
        {
            ...
            switch (_kind)
            {
                ...
                case MultinameLA:
                    ...
                    break;
                case TypeName:
                    uint n=_data[1].Value;
                    Primitives.WriteU30(output, _data[0]);
                    Primitives.WriteU30(output, _data[1]);
                    for (uint i = 0; i < n; ++i)
                    {
                        Primitives.WriteU30(output,_data[i + 2]);
                    }
                    break;
                default:
                    Console.WriteLine(Kind);
