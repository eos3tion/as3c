...
namespace SwfLibrary.Abc
{
    public class MethodBodyInfo : IExternalizeable, IHasTraits
    {
        ...
        protected List<TraitInfo> _traits;
        ...
        /// </summary>
        /// 
        protected List<AVM2Command> instructions;
        public List<AVM2Command> newInstructions;

        public List<AVM2Command> Instructions
        {
            get {
                if (instructions == null)
                {
                    instructions = new List<AVM2Command>();
                }
                return instructions;
            }
        }
        protected List<JumpTargetData> jumpTargets;

        public List<JumpTargetData> JumpTargets
        {
            get {
                if (jumpTargets == null)
                {
                    jumpTargets = new List<JumpTargetData>();
                }
                return jumpTargets; }
        }
        ...
        public void ReadExternal(BinaryReader input)
        {
            ...
            n = Primitives.ReadU30(input).Value;
            _exceptionInfo = new List<ExceptionInfo>(Capacity.Max(n));
            for (uint i = 0; i < n; ++i)
            {
            ...
            _traits = new List<TraitInfo>(Capacity.Max(n));
        ...
        public void WriteExternal(BinaryWriter output)
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                _exceptionInfo[i].WriteExternal(output);
            }
            ...
            for (int i = 0; i < n; ++i)
            {
                _traits[i].WriteExternal(output);
            }
        }
